...
export class AIPlayer {
  ...
  async makeMove(board: GameBoard, isFirstMove: boolean = false, lastPlayerMove?: number): Promise<number> {
    ...
    // Check for player fork (2+ winning threats) - CRITICAL for high-level AI
    // This must come before simple blocking to prevent fork setups
    if (this.difficulty >= 4) {
      const forkBlockMove = this.detectPlayerFork(board, availableColumns);
      if (forkBlockMove !== -1) {
        this.aiMoveHistory.push(forkBlockMove);
  ...
  private detectPlayerFork(board: GameBoard, availableColumns: number[]): number {
    // Detect if player is setting up a fork (a move that creates 2+ winning threats)
    // This is critical for high-level AI to prevent the classic fork strategy

    const forkThreats: { column: number; winCount: number }[] = [];

    for (const col of availableColumns) {
      const testBoard = dropPiece(board, col, 'player');
      if (!testBoard) continue;

      // Count how many winning moves the player would have after this move
      let winningMoveCount = 0;
      const nextAvailable = getAvailableColumns(testBoard);

      for (const nextCol of nextAvailable) {
        const nextBoard = dropPiece(testBoard, nextCol, 'player');
        if (nextBoard && checkWinner(nextBoard) === 'player') {
          winningMoveCount++;
        }
      }

      // If player can create 2+ winning moves, this is a fork threat
      if (winningMoveCount >= 2) {
        forkThreats.push({ column: col, winCount: winningMoveCount });
      }
    }

    // Sort by number of winning moves (block the most dangerous fork first)
    if (forkThreats.length > 0) {
      forkThreats.sort((a, b) => b.winCount - a.winCount);

      // Difficulty-based fork blocking probability
      let blockChance = 0;
      if (this.difficulty >= 9) {
        blockChance = 0.99; // Legend: almost always block forks
      } else if (this.difficulty >= 7) {
        blockChance = 0.95; // Champion/GC: very reliable
      } else if (this.difficulty >= 5) {
        blockChance = 0.85; // Platinum/Diamond: mostly block
      } else if (this.difficulty >= 4) {
        blockChance = 0.70; // Gold: sometimes block
      }

      // Only block if random roll succeeds
      if (Math.random() < blockChance) {
        return forkThreats[0].column;
      }
    }
